...
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Why Us Content</h4>
                        <h4 class="card-title">
                            <a class="add-menu-sidebar" href="{{ route('admin.whyus.create') }}">Create New</a>
                        </h4>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="datatable" class="display min-w850">
                                ...
                                <tbody>
                                    @foreach ($why as $why)
                                        <tr data-identifier="{{ $why->id }}">
                                            <td></td>
                                            <td>{{ $why->title }}</td>
                                            <td>{{ $why->short_description }}</td>
                                            {{-- <td>{{ $why->position }}</td> --}}
                                            <td>
                                                <div class="d-flex flex-column flex-md-row justify-content-center"
                                                    style="gap: 0.5rem">
                                                    @can('whyus update')
                                                        <a href="{{ route('admin.whyus.edit', $why->id) }}"
                                            ...
                                            <form action="{{ route('admin.whyus.destroy', $why->id) }}"
                                                method="post" id="destroy-{{ $why->id }}" style="display: none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </tr>
    ...
    <form action="{{ route('admin.whyus.bulkDelete') }}" id="deleteSelectedForm" method="POST" class="d-none">
        @csrf
        @method('DELETE')
