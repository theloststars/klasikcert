...
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <h4 class="card-title">
                            <a class="add-menu-sidebar" href="{{ route('admin.contacts.create') }}">Create New</a>
                        </h4>
                    </div>
                    <div class="card-body">
                            ...
                            <table id="datatable" class="display min-w850">
                                ...
                                <tbody>
                                    @foreach ($contacts as $contact)
                                        <tr data-identifier="{{ $contact->id }}">
                                            <td></td>
                                            <td>{{ $contact->title }}</td>
                                            <td>{{ $contact->url_slug }}</td>
                                            ...
                                            <form action="{{ route('admin.contacts.destroy', $contact->id) }}"
                                                method="post" id="destroy-{{ $contact->id }}" style="display: none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </tr>
    ...
    <form action="{{ route('admin.contacts.bulkDelete') }}" id="deleteSelectedForm" method="POST" class="d-none">
        @csrf
        @method('DELETE')
