                        ...
                        <form action="{{ route('admin.contacts.update', $contact->id) }}" method="POST"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="title" name="title" placeholder="Title"
                                    style="color: #656773;" value="{{ $contact->title }}" required>
                                @error('title')
                                    <div class="text-sm text-danger">{{ $message ?? 'Something error' }}</div>
                                @enderror
                            </div>
                            <div class="form-group">
                                <label for="title" class="text-capitalize">URL-SLUG</label>
                                <input type="text" class="form-control" id="url_slug" name="url_slug" placeholder="Title"
                                    style="color: #656773;" value="{{ $contact->url_slug }}" required>
                                @error('title')
                                    <div class="text-sm text-danger">{{ $message ?? 'Something error' }}</div>
                                @enderror
                            </div>
                            <div class="form-group">
                                <label for="title" class="text-capitalize">content</label>
                                <input type="text" class="form-control" id="content" name="content" placeholder="Title"
                                    style="color: #656773;" value="{{ $contact->content }}" required>
                                @error('title')
                                    <div class="text-sm text-danger">{{ $message ?? 'Something error' }}</div>
                                @enderror
                            </div>
