    ...
    <div class="container-fluid">
        <div class="page-titles">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('admin.abouts.index') }}">About Menu</a></li>
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.abouts.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="short_description" id="short_description" rows="10" class="form-control" >{!! old('short_description') !!}</textarea>
                                @error('short_description')
                                    <div class="text-sm text-danger">{{ $message ?? 'Something error' }}</div>
                                @enderror
                            </div>
                            <div class="form-group">
                                <label for="body" class="text-capitalize">Description</label>
                                <textarea name="description" id="body" rows="10" class="form-control">{!! old('short_description') !!}</textarea>
                                @error('description')
                                    <div class="text-sm text-danger">{{ $message ?? 'Something error' }}</div>
                                @enderror
                            </div>
