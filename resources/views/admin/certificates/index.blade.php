...
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <h4 class="card-title">
                            <a class="add-menu-sidebar" href="{{ route('admin.certificates.create') }}">Create New</a>
                        </h4>
                    </div>
                    <div class="card-body">
                            ...
                            <table id="datatable" class="display min-w850">
                                ...
                                <tbody>
                                    @foreach ($certificates as $certificate)
                                        <tr data-identifier="{{ $certificate->id }}">
                                            <td></td>
                                            ...
                                            {{-- <td>
                                                <span style="word-break: break-all;">{{ $certificate->scope }}</span>
                                            </td> --}}
                                            <td>{{ \Carbon\Carbon::parse($certificate->issue_date)->format('d-m-Y') }}</td>
                                            <td>{{ \Carbon\Carbon::parse($certificate->expiry_date)->format('d-m-Y') }}
                                            </td>
                                            <td>{{ $certificate->location }}</td>
                                            ...
                                            <form action="{{ route('admin.certificates.destroy', $certificate->id) }}"
                                                method="post" id="destroy-{{ $certificate->id }}" style="display: none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </tr>
    ...
    <form action="{{ route('admin.certificates.bulkDelete') }}" id="deleteSelectedForm" method="POST" class="d-none">
        @csrf
        @method('DELETE')
