...
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <h4 class="card-title">
                            <a class="add-menu-sidebar" href="{{ route('admin.feedback.create') }}">Create New</a>
                        </h4>
                    </div>
                    <div class="card-body">
                            ...
                            <table id="datatable" class="display min-w850">
                                ...
                                <tbody>
                                    @foreach ($feedbacks as $feedback)
                                        <tr data-identifier="{{ $feedback->id }}">
                                            <td></td>
                                            <td>{{ $feedback->name }}</td>
                                            <td>
                                                <img src="{{ Storage::disk('public')->url($feedback->image) }}"
                                                    style="width: 100px;">
                                            </td>
                                            ...
                                            <form action="{{ route('admin.feedback.destroy', $feedback->id) }}"
                                                method="post" id="destroy-{{ $feedback->id }}" style="display: none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </tr>
    ...
    <form action="{{ route('admin.feedback.bulkDelete') }}" id="deleteSelectedForm" method="POST" class="d-none">
        @csrf
        @method('DELETE')
